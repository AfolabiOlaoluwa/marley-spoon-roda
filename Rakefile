...
task :setup, [:app] do |_, args|
  unless (name = args[:app])
    warn "ERROR: Must provide a name argument: example: rake \"setup[App]\""
    exit(1)
  end
...
when 'test'
  ENV['#{upper_name}_SESSION_SECRET'] ||= #{random_bytes.call}.unpack('m')[0] # rubocop:disable Layout/LineLength
when 'production'
  ENV['#{upper_name}_SESSION_SECRET'] ||= #{random_bytes.call}.unpack('m')[0] # rubocop:disable Layout/LineLength
else
  ENV['#{upper_name}_SESSION_SECRET'] ||= #{random_bytes.call}.unpack('m')[0] # rubocop:disable Layout/LineLength
